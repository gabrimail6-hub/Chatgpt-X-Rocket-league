...
namespace
{
    ...
    void fillRoundedBox(CanvasWrapper& canvas, Vector2 origin, Vector2 size, int radius)
    {
        radius = std::max(0, std::min(radius, std::min(size.X, size.Y) / 2));
        if (radius <= 1)
        {
...
void TakeoffCoach::registerCvars()
{
    ...
    reg("tc_reaction_allowance_ms", "100", 0.0f, 1000.0f);
    ...
    reg("tc_car_speed_min", "1500", -1800.0f, 2300.0f);
    reg("tc_car_speed_max", "2300", -1800.0f, 2300.0f);
    reg("tc_ball_speed_min", "500", 0.0f, 2200.0f);
    reg("tc_ball_speed_max", "1500", 0.0f, 2200.0f);
    reg("tc_ball_vertical_min", "0", -1600.0f, 1600.0f);
    ...
    reg("tc_min_initial_jump_delay_ms", "1500", 0.0f, 6000.0f);
    ...
    reg("tc_target_height_min", "600", 93.0f, 1951.0f);
    reg("tc_target_height_max", "685", 93.0f, 1951.0f);
    reg("tc_height_yellow_margin", "180", 20.0f, 600.0f);


    reg("tc_show_hud", "1", 0.0f, 1.0f);
    reg("tc_show_numbers", "1", 0.0f, 1.0f);
    reg("tc_show_indicators", "1", 0.0f, 1.0f);
    ...
    reg("tc_shoot_car_speed_min", "1500", -1800.0f, 2300.0f);
    reg("tc_shoot_car_speed_max", "2300", -1800.0f, 2300.0f);
    reg("tc_shoot_ball_speed_min", "500", 0.0f, 2200.0f);
    reg("tc_shoot_ball_speed_max", "1500", 0.0f, 2200.0f);
    reg("tc_shoot_ball_vertical_min", "0", -1600.0f, 1800.0f);
    ...
    reg("tc_fast_car_speed_min", "900", -1800.0f, 2300.0f);
    reg("tc_fast_car_speed_max", "2300", -1800.0f, 2300.0f);
    ...
    reg("tc_random_distance_min", "-1800", -1800.0f, 6000.0f);
    reg("tc_random_distance_max", "6000", -1800.0f, 6000.0f);
    reg("tc_random_lateral_min", "-2200", -2200.0f, 2200.0f);
    reg("tc_random_lateral_max", "2200", -2200.0f, 2200.0f);
    reg("tc_random_ball_height_min", "93", 93.0f, 1951.0f);
    reg("tc_random_ball_height_max", "1951", 93.0f, 1951.0f);
    reg("tc_random_setup_rotation_min", "-180", -180.0f, 180.0f);
    reg("tc_random_setup_rotation_max", "180", -180.0f, 180.0f);
    reg("tc_random_car_facing_min", "-90", -180.0f, 180.0f);
    reg("tc_random_car_facing_max", "90", -180.0f, 180.0f);
    reg("tc_random_car_speed_min", "-1800", -1800.0f, 2300.0f);
    reg("tc_random_car_speed_max", "2300", -1800.0f, 2300.0f);
    reg("tc_random_ball_speed_min", "0", 0.0f, 2200.0f);
    reg("tc_random_ball_speed_max", "2200", 0.0f, 2200.0f);
    reg("tc_random_ball_vertical_min", "-1600", -1600.0f, 1800.0f);
    reg("tc_random_ball_vertical_max", "1800", -1600.0f, 1800.0f);
    reg("tc_random_ball_direction_min", "-180", -180.0f, 180.0f);
    reg("tc_random_ball_direction_max", "180", -180.0f, 180.0f);

    constexpr unsigned char permission = PERMISSION_FREEPLAY;

    cvarManager->registerNotifier(
        "tc_new",
        [this](std::vector<std::string>) { requestManualScenario(); },
...
void TakeoffCoach::clearAutomaticRerollLatch()
{
    consecutiveRerolls_ = 0;
    automaticRerollsStopped_ = false;
}
...
void TakeoffCoach::requestManualScenario()
{
    ++currentGeneration_;
    clearAutomaticRerollLatch();
    attempt_ = Attempt{};
    attempt_.generation = currentGeneration_;
...
void TakeoffCoach::requestAutomaticReroll()
{
    const int maximum = std::max(1, getInt("tc_max_setup_rejections"));
    if (automaticRerollsStopped_)
        return;
    if (consecutiveRerolls_ >= maximum)
    {
        automaticRerollsStopped_ = true;
        attempt_.phase = Phase::Idle;
        attempt_.validationState = ValidationState::Failed;
        attempt_.headline = "Automatic rerolls stopped. Press New Scenario.";
        attempt_.lastFailureReason = "Maximum automatic rerolls reached";
        return;
    }
    ++consecutiveRerolls_;
    ++currentGeneration_;
    const uint64_t generation = currentGeneration_;
    attempt_.generation = generation;
    gameWrapper->SetTimeout(
        [this, generation](GameWrapper*)
        {
            if (generation == currentGeneration_)
...
void TakeoffCoach::startScenarioNow(uint64_t generation)
{
    ...
    for (int i = 0; i < 50 && !success; ++i)
        success = generateScenario(scenario, validation);

    if (!success || !validation.valid || !applyScenario(scenario))
    {
        attempt_.phase = Phase::Idle;
        attempt_.headline = "Could not generate a validated scenario; retrying.";
        attempt_.lastFailureReason = "Generation validation failed";
        requestAutomaticReroll();
        return;
    }
    ...
    const float relativeContact = validation.target.contactAbsoluteTime;
    validation.target.contactAbsoluteTime = attempt_.startedAt + relativeContact;
    validation.reach.target = validation.target;
    validation.reach.idealJumpAbsolute = attempt_.startedAt + validation.reach.idealJumpAbsolute;
    validation.reach.jumpDelay = validation.reach.idealJumpAbsolute - attempt_.startedAt;
    validation.reach.contactDelay = relativeContact;
    validation.reach.contactPoint = validation.target.desiredCarPosition;
...
bool TakeoffCoach::generateScenario(Scenario& scenario, ScenarioValidation& validation)
{
    ...
    const float sampledDistance = sample("tc_distance_min", "tc_distance_max");
    const float distance = scenario.objective == Objective::FullRandom ? sampledDistance : std::max(350.0f, sampledDistance);
    ...
    if (scenario.objective == Objective::Shoot && ballVertical > 0.0f)
    {
        ...
        const float safeHeadroom = std::max(0.0f, highestAllowedCenter - ballHeight);
        const float safeVz = std::sqrt(2.0f * 650.0f * safeHeadroom);
        if (ballVertical > safeVz)
        {
    ...
    const float predictionDt = getFloat("tc_prediction_timestep");

    Vector contact{};
    bool foundContact = false;
    Vector predictedPosition = scenario.ballPosition;
    Vector predictedVelocity = scenario.ballVelocity;
    Vector predictedAngular{};
    ...
    for (float t = 0.0f; t <= maxContact; t += predictionDt)
    {
        if (t >= minContact
            ...
            && !isUnsupportedTransition(predictedPosition, 93.0f))
        {
            contact = predictedPosition;
            foundContact = true;
            break;
        }

        predictedVelocity.Z += GRAVITY_Z * predictionDt;
        Vector next = predictedPosition + predictedVelocity * predictionDt;
        SurfaceType surface = SurfaceType::None;
        if (collideBall(next, predictedVelocity, predictedAngular, 93.0f, surface))
        {
            ++predictedBounces;
            if (predictedBounces > maximumBounces)
                break;
        }
    ...
    scenario.carVelocity = facing * carSpeed;
...
TakeoffCoach::ScenarioValidation TakeoffCoach::validateScenario(const Scenario& scenario) const
{
    ScenarioValidation out;
    if (std::abs(scenario.carPosition.X) > 3900.0f || std::abs(scenario.carPosition.Y) > 5000.0f)
        return out;
    if (scenario.ballPosition.Z < 93.0f || scenario.ballPosition.Z > 1951.0f)
        return out;
    ...
    const float minContact = getFloat("tc_contact_min");
    const float horizon = std::min(10.0f, getFloat("tc_prediction_horizon"));
    const float dt = getFloat("tc_prediction_timestep");
    const int maximumBounces = std::max(0, getInt("tc_max_bounces"));
    const float minimumPreparation = getFloat("tc_min_initial_jump_delay_ms") / 1000.0f;
    ...
    for (float time = 0.0f; time <= horizon; time += dt)
    {
        ...
        slice.supportedGeometry = !isUnsupportedTransition(position, 93.0f);
        out.path.push_back(slice);

        if (!slice.supportedGeometry)
        {
            out.reason = RejectionReason::PredictionLowConfidence;
            return out;
        }
        ...
        {
            sawHeight = true;
            if (scenario.objective != Objective::Shoot || velocity.Z < 0.0f)
            {
                ContactTarget target;
                target.valid = true;
                target.contactAbsoluteTime = time;
                target.ballPosition = position;
                target.ballVelocity = velocity;
                target.bounceCount = bounceCount;
                target.lastSurface = lastBounce;

                if (scenario.objective == Objective::Shoot)
                {
                    const Vector goalPoint{0.0f, static_cast<float>(scenario.goalSign) * 5120.0f, 320.0f};
                    const Vector goalDirection = normalized2D(goalPoint - position);
                    target.desiredGoalPoint = goalPoint;
                    target.desiredFacingDirection = goalDirection;
                    target.desiredCarPosition = position - goalDirection * 145.0f - Vector{0.0f, 0.0f, 45.0f};
                }
                else
                {
                    target.desiredFacingDirection = normalized2D(position - scenario.carPosition);
                    target.desiredCarPosition = position - target.desiredFacingDirection * 145.0f - Vector{0.0f, 0.0f, 45.0f};
                }

                if (std::abs(target.desiredCarPosition.X) > 4000.0f
                    || std::abs(target.desiredCarPosition.Y) > 5050.0f)
                {
                    out.reason = RejectionReason::InvalidShootApproach;
                    return out;
                }

                const float available = time;
                const float vertical = target.desiredCarPosition.Z - scenario.carPosition.Z;
                const float estimatedAerial = clamp(
                    0.42f + std::max(0.0f, vertical) / 1350.0f,
                    0.32f,
                    std::max(0.32f, available));
                const float idealJumpDelay = available - estimatedAerial;
                if (idealJumpDelay >= minimumPreparation)
                {
                const Vector route = normalized2D(target.desiredCarPosition - scenario.carPosition);
                const Vector nose = normalized2D(forwardFromRotator(scenario.carRotation));
                const float forwardSpeed = dot(scenario.carVelocity, route);
                const Vector lateralVelocity = scenario.carVelocity - route * forwardSpeed;
                const float lateralSpeed = length2D(lateralVelocity);
                const float headingError = std::abs(signedAngleDeg2D(nose, route));
                const float correctionTime =
                    headingError / 105.0f
                    + lateralSpeed / 1500.0f;
                const float usefulGroundTime = std::max(0.0f, idealJumpDelay - correctionTime);
                const float horizontal = length2D(target.desiredCarPosition - scenario.carPosition);
                const float horizontalReach =
                    std::max(0.0f, forwardSpeed) * idealJumpDelay
                    + 0.5f * 900.0f * usefulGroundTime * usefulGroundTime
                    + 260.0f;
                const float verticalReach =
                    300.0f + 620.0f * estimatedAerial
                    + 0.5f * 1000.0f * estimatedAerial * estimatedAerial;

                if (horizontal <= horizontalReach && vertical <= verticalReach)
                {
                    sawReachableHeight = true;
                    out.target = target;
                    out.reach.valid = true;
                    out.reach.target = target;
                    out.reach.contactPoint = target.desiredCarPosition;
                    out.reach.requiredDirection = route;
                    out.reach.requiredAerialDuration = estimatedAerial;
                    out.reach.idealJumpAbsolute = idealJumpDelay;
                    out.reach.jumpDelay = idealJumpDelay;
                    out.reach.contactDelay = available;
                    out.reach.confidence = clamp(1.0f - correctionTime / std::max(0.25f, idealJumpDelay), 0.25f, 0.85f);
                    out.reach.robustness = out.reach.confidence * 0.75f;
                    out.reach.simpleFallback = true;
                    out.reach.idealTakeoffPosition =
                        scenario.carPosition + scenario.carVelocity * idealJumpDelay;
                    out.reach.idealTakeoffPosition.Z = scenario.carPosition.Z;
                    out.reach.takeoff = TakeoffState{
                        out.reach.idealTakeoffPosition,
                        scenario.carVelocity,
                        nose,
                        idealJumpDelay};
                    out.valid = true;
                    out.reason = RejectionReason::None;
                    return out;
                }
                }
            }
        ...
        Vector next = position + velocity * dt;
        SurfaceType surface = SurfaceType::None;
        if (collideBall(next, velocity, angularVelocity, 93.0f, surface))
        {
            ++bounceCount;
            lastBounce = surface;
            if (bounceCount > maximumBounces)
            {
                out.reason = RejectionReason::TargetAfterFirstCollision;
                return out;
            }
        }
        position = next;
    ...
    if (!sawHeight)
        out.reason = RejectionReason::NoTargetInHeightBand;
    else if (!sawReachableHeight)
        out.reason = RejectionReason::UnreachableTarget;
    return out;
}
...
void TakeoffCoach::updateAttempt(CarWrapper car, float now)
{
    ...
    {
        const BounceEvent* nearestPredictedBounce = nullptr;
        float nearestTimeError = 999.0f;
        for (const BounceEvent& event : attempt_.bounceEvents)
        {
            const float timeError = std::abs(event.absoluteTime - now);
            if (timeError < nearestTimeError)
            {
                nearestTimeError = timeError;
                nearestPredictedBounce = &event;
            }
        }

        // A velocity discontinuity is only treated as a bounce mismatch when
        // there was a predicted bounce at the same time and place. Ordinary
        // gravity, soft touches and harmless numerical drift never invalidate
        // timing by themselves.
        if (nearestPredictedBounce != nullptr && nearestTimeError <= 0.14f)
        {
            const float positionError =
                length(ball.GetLocation() - nearestPredictedBounce->position);
            const float outgoingVelocityError =
                length(currentVelocity - nearestPredictedBounce->outgoingVelocity);
            const bool mismatch =
                positionError > getFloat("tc_ball_path_position_tolerance") * 2.25f
                || outgoingVelocityError
                    > getFloat("tc_ball_path_velocity_tolerance") * 2.25f;
            if (mismatch)
            {
                attempt_.predictedBounceMismatch = true;
                attempt_.pathStillValid = false;
            }
        }
    }
    ...
    {
        if (getBool("tc_auto_reset")) requestAutomaticReroll();
    ...
    if (now - attempt_.lastSolveAt >= 1.0f / hz)
    {
        ...
        if (!attempt_.targetLocked)
        {
            if (attempt_.ballPath.empty())
            {
                ...
                if (attempt_.validationCandidate.valid)
                {
                    const float allowance = attempt_.guidanceStyle == GuidanceStyle::ReactionCue
                        ? getFloat("tc_reaction_allowance_ms") / 1000.0f : 0.0f;
                    const float minimumPreparation =
                        getFloat("tc_min_initial_jump_delay_ms") / 1000.0f;
                    const float requiredJumpDelay =
                        std::max(minimumPreparation, allowance + 0.10f);
                    if (attempt_.initialAvailableJumpDelay < requiredJumpDelay)
                        attempt_.validationCandidate = Solution{};
                }
    ...
    float now)
{
    ...
    if (attempt_.guidanceStyle == GuidanceStyle::ReactionCue && attempt_.reactionCaptured)
    {
        ...
        if (touched && attempt_.possibleTimeSavedMs >= 0.0f)
            ...
            attempt_.correction += " Contact timing —";
        else
    ...
    if (attempt_.everHadSolution && timing < 9000.0f)
        ...
        detail << (attempt_.predictedBounceMismatch ? "—" : "n/a");
...
bool TakeoffCoach::cheapReachable(CarWrapper car, const ContactTarget& target, float now) const
{
    ...
    const Vector route = normalized2D(delta);
    const Vector velocity = car.GetVelocity();
    const float forwardSpeed = dot(velocity, route);
    const Vector lateralVelocity = velocity - route * forwardSpeed;
    const float lateralSpeed = length2D(lateralVelocity);
    const Vector nose = normalized2D(forwardFromRotator(car.GetRotation()));
    const float headingError = std::abs(signedAngleDeg2D(nose, route));

    const float correctionTime =
        headingError / 105.0f
        + lateralSpeed / 1500.0f;
    const float usefulTime = std::max(0.0f, delay - correctionTime);
    const float horizontalReach =
        std::max(0.0f, forwardSpeed) * delay
        + 0.5f * 900.0f * getFloat("tc_horizontal_calibration")
            * usefulTime * usefulTime
        + 180.0f;
    const float verticalReach = std::max(
        0.0f,
        292.0f * getFloat("tc_vertical_calibration") * delay
        + 0.5f
            * (BOOST_ACCEL * getFloat("tc_vertical_calibration") + 650.0f)
            * delay * delay);

    return horizontal <= horizontalReach
        && vertical <= verticalReach + 180.0f;
}
...
void TakeoffCoach::renderHud(CanvasWrapper canvas)
{
    ...
    const bool feedback =
        attempt_.phase == Phase::Feedback;
    const bool indicatorsEnabled = getBool("tc_show_indicators");
    ...
    fillRoundedBox(canvas, origin, Vector2{panelWidth, panelHeight}, 12);
    ...
    if (indicatorsEnabled && timingVisible && getBool("tc_show_timing"))
    {
        drawGauge(
            canvas,
    ...
    if (indicatorsEnabled && alignmentVisible && getBool("tc_show_alignment"))
    {
        drawGauge(
            canvas,
            gaugeOrigin(gaugeIndex++),
    ...
    if (cueMode && getBool("tc_show_reaction_cue") && !feedback)
    {
        ...
        fillRoundedBox(canvas, Vector2{cx - width / 2, cy - height / 2}, Vector2{width, height}, 10);
    }
        ...
        && getBool("tc_show_takeoff_marker") && !feedback)
    {
        ...
        takeoff.Z = 17.0f;
        const Vector direction = normalized2D(visualTarget.target.desiredCarPosition - takeoff);
...
{
    if (preset == 0) return "tc_shoot_";
    if (preset == 1) return "tc_fast_";
    return "tc_random_";
}
...
void TakeoffCoach::resetModePreset(int preset)
{
    ...
    if (preset == 0)
    {
        ...
        setValue("tc_shoot_ball_vertical_min",0.0f); setValue("tc_shoot_ball_vertical_max",1600.0f);
    ...
    {
        setValue("tc_random_distance_min",-1800.0f); setValue("tc_random_distance_max",6000.0f);
        setValue("tc_random_lateral_min",-2200.0f); setValue("tc_random_lateral_max",2200.0f);
        setValue("tc_random_ball_height_min",93.0f); setValue("tc_random_ball_height_max",1951.0f);
        setValue("tc_random_setup_rotation_min",-180.0f); setValue("tc_random_setup_rotation_max",180.0f);
        setValue("tc_random_car_facing_min",-90.0f); setValue("tc_random_car_facing_max",90.0f);
        setValue("tc_random_car_speed_min",-1800.0f); setValue("tc_random_car_speed_max",2300.0f);
        setValue("tc_random_ball_speed_min",0.0f); setValue("tc_random_ball_speed_max",2200.0f);
        setValue("tc_random_ball_vertical_min",-1600.0f); setValue("tc_random_ball_vertical_max",1800.0f);
        setValue("tc_random_ball_direction_min",-180.0f); setValue("tc_random_ball_direction_max",180.0f);
    }
    if (preset >= 0 && preset <= 2 && getInt("tc_setup_preset") == preset) loadModePreset(preset);
}

void TakeoffCoach::renderDrillTab()
{
    int objective = getInt("tc_objective");
    const char* objectives[] = {"Fast Touch", "Shoot", "Full Random"};
    if (ImGui::Combo("Objective", &objective, objectives, 3))
    {
        setValue("tc_objective", objective);
        clearAutomaticRerollLatch();
        if (getBool("tc_auto_load_matching_setup"))
            applyPreset(objective == static_cast<int>(Objective::Shoot) ? 0 : (objective == static_cast<int>(Objective::FullRandom) ? 2 : 1));
    }
    ...
    float minimumJumpDelay = getFloat("tc_min_initial_jump_delay_ms");
    if (ImGui::SliderFloat("Minimum preparation time before jump", &minimumJumpDelay, 0.0f, 6000.0f, "%.0f ms"))
        setValue("tc_min_initial_jump_delay_ms", minimumJumpDelay);
    ImGui::TextWrapped("Reaction allowance is added on top, so the green cue remains visible before the accepted jump time.");

    rangeControl("Ball-centre target height", "tc_target_height_min", "tc_target_height_max",
        93.0f, 1951.0f, "%.0f uu");
    ImGui::TextUnformatted("Ground contact: 93 uu | Ceiling contact: 1951 uu");

    if (guidance == 1)
    {
        float allowance = getFloat("tc_reaction_allowance_ms");
        if (ImGui::SliderFloat("Human reaction allowance", &allowance, 0.0f, 1000.0f, "%.0f ms"))
    ...
    ImGui::Separator();
    if (objective == 0)
        ImGui::TextWrapped("Fast Touch ignores both goals and selects the earliest useful reachable touch.");
    else if (objective == 1)
        ImGui::TextWrapped("Shoot builds a coherent scoring approach around a descending target.");
    else
        ImGui::TextWrapped("Full Random allows chaotic approaches, reverse speed, overhead and unfavorable situations while keeping a selectable target.");
...
void TakeoffCoach::renderSetupTab()
{
    ...
    const char* presets[] = {"Shoot Setup", "Fast Touch Setup", "Full Random Setup"};
    ...
    ImGui::SameLine();
    if (ImGui::Button("Reset Full Random")) resetModePreset(2);
    ...
    rangeControl("Car speed", "tc_car_speed_min", "tc_car_speed_max",
        -1800.0f, 2300.0f, "%.0f uu/s");
    ImGui::TextWrapped("Negative speed spawns the car moving backwards along the intended approach line.");
...
void TakeoffCoach::renderFeedbackTab()
{
    ...
    bool showIndicators = getBool("tc_show_indicators");
    if (ImGui::Checkbox("Show timing / alignment / height indicators", &showIndicators))
        setValue("tc_show_indicators", showIndicators);
...
void TakeoffCoach::renderAdvancedTab()
{
    ...
    ImGui::Text("Prediction horizon: %.1f s (fixed)", horizon);
...
{
    const int value = getInt("tc_objective");
    if (value == 1) return Objective::Shoot;
    if (value == 2) return Objective::FullRandom;
    return Objective::FastTouch;
}

std::string TakeoffCoach::objectiveName(Objective objective)
{
    if (objective == Objective::Shoot) return "SHOOT";
    if (objective == Objective::FullRandom) return "FULL RANDOM";
    return "FAST TOUCH";
}
